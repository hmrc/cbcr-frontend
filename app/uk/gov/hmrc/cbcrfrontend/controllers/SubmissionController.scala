...
class SubmissionController @Inject()(val messagesApi: MessagesApi,
  ...
  def confirm = Action.async{ implicit request =>
    authorised().retrieve(Retrievals.credentials) { creds =>
      (for {
        ...
        _ <- createSuccessfulSubmissionAuditEvent(creds, summaryData)
  ...
  def submitSuccessReceipt = Action.async { implicit request =>
    authorised() {
      ...
      data.fold[Result](
        ...
        tuple => Ok(views.html.submission.submitSuccessReceipt( tuple._2, tuple._1.value))
      )
    }
  }
